...
pred_pre_list_df$p_reg  <- apply(sim_data,1,function(pred){
  t(biom - pred) %*% 
    solve(0.5*diag(diag(var(sim_data)))) %*% 
    (biom - pred)
})>3
...
new_pred_pre_list_jitter <- sapply(1:ncol(new_pred_pre_list),function(x) {
    x_old <- new_pred_pre_list[,x]
...
ggplot(mp,aes(x=value)) + geom_histogram(aes(x=value)) + facet_wrap(~Var2,scales='free')

                        
new_pred_list <- split(data.frame(new_pred_pre_list_jitter),1:nrow(new_pred_pre_list))       
system.time( new_simdat <- parallel::mclapply(new_pred_list,run_SS_sim,mc.cores = 4))
...
testset1 <- data.frame(h=seq(35,66,l=4),
                       r_pp = 10,
                       sigma= 1.3)
