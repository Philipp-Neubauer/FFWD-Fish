...
system.time( simdat <- parallel::mclapply(pred_list,run_SS_sim,mc.cores = 8))
...
pred_pre_list_df$p_reg <- apply(sim_data,1,function(x) any(x<(min(biom)/100)))
...
pred_pre_list_df$p_reg  <- apply(sim_data,1,function(pred){
  t(biom - pred) %*% 
    solve(0.1*diag(diag(var(sim_data)))) %*% 
    (biom - pred)
})>3
...
new_pred_pre_list <- expand.grid(new_preds)
new_pred_pre_list_jitter <- apply(new_pred_pre_list,2,function(x) x+rnorm(length(x),0,sd(x)/5))
new_pred_pre_list_jitter[new_pred_pre_list_jitter<0] <- new_pred_pre_list[new_pred_pre_list_jitter<0]                                     
colnames(new_pred_pre_list_jitter) <- c('h','r_pp','sigma')

new_pred_list <- split(data.frame(new_pred_pre_list_jitter),1:nrow(new_pred_pre_list))       
system.time( new_simdat <- parallel::mclapply(new_pred_list,run_SS_sim,mc.cores = 8))

new_sim_data <- do.call('rbind',new_simdat)

pred_pre_list_df <- data.frame(new_pred_pre_list_jitter)
pred_pre_list_df$p_reg <- apply(new_sim_data,1,function(x) any(x<(min(biom)/100)))
...
cube <- 4
testset1 <- data.frame(h=seq(30,50,l=8),
                         r_pp = 10,
                         sigma= 1.3)
...
colnames(testdata1) <- c('h','Species','Biomass')
testdata1$h <- testset1$h
...
colnames(testdata1_em)[3] <- 'h'
testdata1_em$h <- rep(testset1$h,each=10)
testdata1_em$Species <- 1:10

test1 <- inner_join(testdata1_em,testdata1)
test1$Species <- factor(test1$Species)

ggplot(test1,aes(x=h,y=Biomass)) +
  geom_line(aes(y=Biomass,linetype='Biomass',col=Species)) +
  geom_ribbon(aes(y=mu,ymin=mu-sqrt(sigma),ymax=mu+sqrt(sigma),x=h,fill=Species),alpha=0.2)+
  geom_line(aes(y=mu,x=h,linetype='Predictions',col=Species)) + 
  facet_wrap(~Species) + 
