...
gridExtra::grid.arrange(gg[[1]],gg[[4]],gg[[5]],gg[[3]],gg[[2]],ncol=1)


mobs <- do.call('rbind',)

mobs %>% split(.$Var2) %>% map(~density(.$value))

ddp <- dd$y/(17.4*(10^dd$x)^-1)
ddp=ddp/max(ddp)
dd$y <- dd$y/max(dd$y)
plot((dd$x),ddp)
lines((dd$x),dd$y)
...
slopes=seq(0,2.5,0.1); ddif = slopes
